...
public class ProductDialog extends JDialog {
    ...
    private void build() {
        ...
        form.add(new LabeledField("Product name", nameField));
        form.add(new LabeledField("Category", categoryField));
        form.add(new LabeledField("Quantity in stock", quantityField));
        form.add(new LabeledField("Ideal stock quantity", idealQuantityField));
        form.add(new LabeledField("Unit price", unitPriceField));
        form.add(new LabeledField("Expiration date (yyyy-MM-dd)", expirationDateField));
        form.add(new LabeledField("Registration date (yyyy-MM-dd)", registrationDateField));
        root.add(form, BorderLayout.CENTER);

        JLabel hint = new JLabel("Use dates in yyyy-MM-dd format.");
    ...
    private void save() {
        try {
            ...
            Dialogs.error(this, new IllegalArgumentException("Please check numbers and dates. Dates must use yyyy-MM-dd."));
        }
    }
}
